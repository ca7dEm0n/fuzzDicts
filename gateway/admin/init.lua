...
local function check_token()
    ...
    local token = ngx.var.cookie_gw_auth_key

    if not token then
        token = ngx.var.cookie_auth_key
    end
    if not token then
        token = get_headers()["X-Token"]
    end
...
function _M.init_worker()
    local resources = {
        ...
        codo_rbac = require("admin.codo_rbac")
    }
